...
def fit_one_epoch(net,epoch,epoch_size,epoch_size_val,gen,genval,Epoch,cuda):
    ...
    torch.save(model.state_dict(), 'logs/line_Epoch%d-Total_Loss%.4f-Val_Loss%.4f.pth'%((epoch+1),total_loss/(epoch_size+1),val_toal_loss/(epoch_size_val+1)))
...
if __name__ == "__main__":
    ...
    #   输入图片的大小
    #------------------------------#
    inputs_size = [512,512,3]   # 输入的宽和高必须是32的倍数
    #---------------------#
    #   分类个数+1
    #   2+1
    #---------------------#
    ...
    #   主干网络预训练权重的使用
    #-------------------------------#
    pretrained = False  # TODO 如果想从头开始训练unet网络就设置为true，并把下面载入权重部分注释掉
    #-------------------------------#
    #   Cuda的使用
    #-------------------------------#
    Cuda = True
    ...
    if True:
        lr = 1e-4
        Init_Epoch = 0  # 网络最开始训练的世代
        Interval_Epoch = 25  # 在冻结主干特征提取网络情况下训练的代数
        Batch_size = 2
    ...
    if True:
        lr = 1e-5
        Interval_Epoch = 25
        Epoch = 300  # 解冻之后总共训练的代数，解冻之后还会训练50-25=25个代数
        Batch_size = 2

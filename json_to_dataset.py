...
if __name__ == '__main__':
    jpgs_path = "datasets/JPEGImages"  # 保存图片的位置
    pngs_path = "datasets/SegmentationClass"  # 保存标签图的位置
    ...
    count = os.listdir("./datasets/before/")   # 遍历图片和json文件
    for i in range(0, len(count)):
        path = os.path.join("./datasets/before", count[i])  # 遍历图片和json文件
